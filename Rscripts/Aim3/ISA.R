...
# Extract sample data
sample_metadata <- sample_data(gc_genus_RA)

# Create a new grouping variable combining cancer stage and age group
sample_metadata$Stage_Age <- paste(sample_metadata$`Group`, 
                                   sample_metadata$`Age_Category`, sep = "_")

# ISA
isa_gc <- multipatt(t(otu_table(gc_genus_RA)), 
                    cluster = sample_metadata$Stage_Age)
#isa_gc <- multipatt(t(otu_table(gc_genus_RA)), cluster = sample_data(gc_genus_RA)$`Group`)
summary(isa_gc)
taxtable <- tax_table(gc_final) %>% as.data.frame() %>% rownames_to_column(var="ASV")
...
write.csv(isa_table, "isa_table.csv", row.names = FALSE)
write_xlsx(isa_table, "isa_table.xlsx")
